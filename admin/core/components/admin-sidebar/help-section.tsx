...
// ui
import { GithubIcon, Tooltip } from "@plane/ui";
// helpers
...
const helpOptions = [
  ...
  {
    name: "Report a bug",
    href: "mailto:juanindrian02@gmail.com",
    Icon: Mail,
  },
];
      ...
      <div className={`flex items-center gap-1 ${isSidebarCollapsed ? "flex-col justify-center" : "w-full"}`}>
        <Tooltip tooltipContent="Redirect to plane" position="right" className="ml-4" disabled={!isSidebarCollapsed}>
          ...
          >
            ...
            {!isSidebarCollapsed && "Redirect to proline"}
          </a>
        </Tooltip>
        <Tooltip tooltipContent="Help" position={isSidebarCollapsed ? "right" : "top"} className="ml-4">
          <button
            type="button"
            className={`ml-auto grid place-items-center rounded-md p-1.5 text-custom-text-200 outline-none hover:bg-custom-background-90 hover:text-custom-text-100 ${isSidebarCollapsed ? "w-full" : ""
              }`}
        ...
        <Tooltip tooltipContent="Toggle sidebar" position={isSidebarCollapsed ? "right" : "top"} className="ml-4">
          <button
            type="button"
            className={`grid place-items-center rounded-md p-1.5 text-custom-text-200 outline-none hover:bg-custom-background-90 hover:text-custom-text-100 ${isSidebarCollapsed ? "w-full" : ""
              }`}
